...
// Store owns a SQLite connection and persists application state.
type Store struct{ db *sql.DB }

// PingContext verifies that the owned database connection is usable.
func (s *Store) PingContext(ctx context.Context) error { return s.db.PingContext(ctx) }

// Close releases the owned database connection.
